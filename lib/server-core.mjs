...
let state={config:structuredClone(DEFAULT_CONFIG),revision:1},queue=Promise.resolve();const clients=new Set();
await mkdir(data,{recursive:true});
try{const saved=JSON.parse(await readFile(file,'utf8'));state.config=validateConfig(saved.config);state.revision=Number.isSafeInteger(saved.revision)?saved.revision:1;}catch(e){if(e.code!=='ENOENT'){console.error('无法读取旧设置，原文件已保留，暂时使用默认值。');}}
...
async function handle(req,res){
 ...
 if(req.method==='GET'&&pathname==='/api/health'){json(res,200,{app:'typecast-studio',version:2});return;}
