...
export function drawBehind(c:CanvasRenderingContext2D,config:Config,b:Bounds,time:number,color:string){
 const span=config.hold+(config.animation==='cut'?0:2*config.transition),cycle=Math.max(0,time)/span,t=cycle*Math.PI*2,h=b.bottom-b.top;
 c.save();
 if(config.effect==='fire'){
  c.shadowColor='#ff7327';c.shadowBlur=8;
  const count=Math.min(56,Math.max(12,Math.ceil(b.width/20)));
  for(let i=0;i<count;i++){
   const phase=t*2+i*2.37,x=b.x+(i+.5)*b.width/count,base=b.bottom+5;
   const tall=h*.6+18+12*Math.sin(phase),tip=b.top-8-10*Math.sin(t*3+i*2.37);
  ...
  for(let j=0;j<3;j++){
   ...
   for(let x=0;x<=b.width;x+=7){const y=(b.top+b.bottom)/2+(j-1)*(h*.55)+Math.sin(x*.035-t+j*1.7)*(7+j*2);if(x===0)c.moveTo(b.x+x,y);else c.lineTo(b.x+x,y);}
   c.stroke();
  }
  ...
  for(let i=0;i<count;i++){
   const u=(cycle+i*.61803398875)%1,x=b.x+(i*.754877666%1)*b.width+Math.sin(t*2+i)*5;
