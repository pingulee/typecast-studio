...
export const DEFAULT_CONFIG={lines:['包天服务请联系本人，微信：your_wechat','游戏打法与思路请看群相册，QQ：123456789'],style:'gold',color:'#ffd45b',outline:'#29180d',width:1200,height:176,fontSize:70,stroke:3,depth:3,glow:0,hold:3.2,transition:.4,animation:'slide',align:'center',highlight:true,visible:true,font:'sans',singleMode:'stay'};
export function validateConfig(input){
 ...
 for(const [key,choices] of Object.entries({style:STYLES.map(x=>x.id),animation:ANIMATIONS.map(x=>x[0]),align:['left','center','right'],font:['sans','serif'],singleMode:['stay','loop']})){const value=input[key]??DEFAULT_CONFIG[key];if(!choices.includes(value))throw new Error(`请检查参数 ${key}。`);out[key]=value;}
