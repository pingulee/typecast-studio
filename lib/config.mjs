...
export const DEFAULT_CONFIG={language:'en',lines:['Contact me for bookings, WeChat: your_wechat','Tips and guides in the group album, QQ: 123456789'],style:'gold',color:'#ffd45b',outline:'#29180d',width:1200,height:176,fontSize:70,stroke:3,depth:3,glow:0,hold:3.2,transition:.4,animation:'slide',align:'center',highlight:true,visible:true,font:'sans',singleMode:'stay'};
export function validateConfig(input){
 if(!input||typeof input!=='object'||Array.isArray(input))throw new Error('设置格式不正确。');
 const out={};if(!Array.isArray(input.lines)||input.lines.length<1||input.lines.length>8)throw new Error('支持 1 至 8 条文案，只写一条也可以。');
 ...
 for(const [key,choices] of Object.entries({language:['en','ko','zh-CN'],style:STYLES.map(x=>x.id),animation:ANIMATIONS.map(x=>x[0]),align:['left','center','right'],font:['sans','serif'],singleMode:['stay','loop']})){const value=input[key]??DEFAULT_CONFIG[key];if(!choices.includes(value))throw new Error(`请检查参数 ${key}。`);out[key]=value;}
 ...
 for(const key of ['highlight','visible']){if(typeof input[key]!=='boolean')throw new Error(`请检查参数 ${key}。`);out[key]=input[key];}return out;
}
