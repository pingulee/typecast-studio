...
export async function openCaptureWindow({key='green',width=1200,height=176,dataDir,port=4318}){
 if(process.platform!=='win32')throw new Error('独立字幕窗口仅支持 Windows。');
 if(!['green','blue','black'].includes(key))throw new Error('背景颜色无效。');
 ...
 const url=`http://localhost:${port}/capture?key=${key}`;
