...
const updater=createUpdater({base,dataDir});
const shutdown=()=>{updater.close();app.close();server.close(()=>process.exit(0));server.closeAllConnections();};
const app=await createApplication(base,PORT,{dataDir,updater,initialLanguage:process.argv.find(x=>x.startsWith('--language='))?.slice(11),openCapture:openCaptureWindow,onShutdown:shutdown}),handle=app.handle;
